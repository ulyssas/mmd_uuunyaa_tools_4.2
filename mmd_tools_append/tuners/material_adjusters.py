...
class MaterialAdjusterUtilities(MaterialEditor):
    def check_attached(self, label: str) -> bool:
        return self.find_node(bpy.types.ShaderNodeGroup, label=label) is not None

    def check_attachable(self) -> bool:
        return self.find_active_principled_shader_node() is not None

    def clean_adjusters_node_frame(self) -> bpy.types.NodeFrame:
...
class MaterialAdjusterABC(MaterialAdjusterUtilities):
    ...
    @abstractmethod
    def attach(self) -> bpy.types.ShaderNodeGroup:
        pass
...
class WetAdjuster(MaterialAdjusterABC):
    ...
    def attach(self):
        node_frame = self.get_adjusters_node_frame()
        node_wet_adjuster = self.find_node(bpy.types.ShaderNodeGroup, label=WetAdjuster.get_name(), node_frame=node_frame)

        node_shader = self.find_active_principled_shader_node()
        node_wet_adjuster = self.edit(
            self.get_wet_adjuster_node(),
            {
                "Specular IOR Level": self.to_link_or_value(node_shader.inputs["Specular IOR Level"]),
        ...
        self.edit(
            node_shader,
            {
                "Specular IOR Level": node_wet_adjuster.outputs["Specular IOR Level"],
    ...
    def detach(self):
        ...
        self.edit(
            self.find_active_principled_shader_node(),
            {
                "Specular IOR Level": self.to_link_or_value(node_wet_adjuster.inputs["Specular IOR Level"]),
...
class GlitterAdjuster(MaterialAdjusterABC):
    ...
    def attach(self):
        node_frame = self.get_adjusters_node_frame()
        node_glitter_adjuster = self.find_node(bpy.types.ShaderNodeGroup, label=GlitterAdjuster.get_name(), node_frame=node_frame)

        node_shader = self.find_active_principled_shader_node()
        node_glitter_adjuster = self.edit(
            self.get_glitter_adjuster_node(),
            {
                "Specular IOR Level": self.to_link_or_value(node_shader.inputs["Specular IOR Level"]),
                "Roughness": self.to_link_or_value(node_shader.inputs["Roughness"]),
        ...
        self.edit(
            node_shader,
            {
                "Specular IOR Level": node_glitter_adjuster.outputs["Specular IOR Level"],
    ...
    def detach(self):
        ...
        self.edit(
            self.find_active_principled_shader_node(),
            {
                "Specular IOR Level": self.to_link_or_value(node_glitter_adjuster.inputs["Specular IOR Level"]),
...
class EmissionAdjuster(MaterialAdjusterABC):
    ...
    def attach(self) -> bpy.types.ShaderNodeGroup:
        node_frame = self.get_adjusters_node_frame()
        node_emission_adjuster = self.find_node(bpy.types.ShaderNodeGroup, label=self.get_name(), node_frame=node_frame)
        ...
        self.edit(
            node_shader,
            {
                "Emission Color": node_emission_adjuster.outputs["Emission Color"],
