...
class PyramidMeshEditor(MeshEditor):
    ...
    def setup_cloth_modifier(
        ...
        self.mesh_object.mmd_tools_append_cloth_settings.presets = "PHYSICS_CLOTH_BREAST_PYRAMID"
...
def build_adjacencies(verts: bmesh.types.BMVertSeq, vid2weight: Dict[int, float]) -> Tuple[Dict[Tuple[int, int], float], Dict[int, int]]:
    ...
    return {(vid2uid[from_xid], vid2uid[near_xid]): span for (from_xid, near_xid), span in adjacencies.items()}, vid2uid
...
class MMDAppendPyramidClothAdjuster(bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context):
        ...
        pyramid_cloth_settings = mesh_object.mmd_tools_append_pyramid_cloth_settings
...
class PyramidClothAdjusterSettingsPropertyGroup(bpy.types.PropertyGroup):
    ...
    def register():
        ...
        bpy.types.Object.mmd_tools_append_pyramid_cloth_settings = bpy.props.PointerProperty(type=PyramidClothAdjusterSettingsPropertyGroup)

    @staticmethod
    def unregister():
        del bpy.types.Object.mmd_tools_append_pyramid_cloth_settings
